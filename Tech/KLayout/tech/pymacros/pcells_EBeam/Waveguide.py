...
class Waveguide(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    TECHNOLOGY = get_technology_by_name('EBeam_CMLC')
    self.param("path", self.TypeShape, "Path", default = DPath([DPoint(0,0), DPoint(10,0), DPoint(10,10)], 0.35))
    self.param("radius", self.TypeDouble, "Radius", default = 5)
    self.param("width", self.TypeDouble, "Width", default = 0.35)
    ...
    self.param("widths", self.TypeList, "Widths", default =  [0.35])
    self.param("offsets", self.TypeList, "Offsets", default = [0])
    self.param("CML", self.TypeString, "Compact Model Library (CML)", default = 'EBeam_CMLC')
    self.param("model", self.TypeString, "CML Model name", default = 'cband_te_wg_strip') 
  ...
  def produce_impl(self):
    ...
    self.cell.shapes(LayerPinRecN).insert(Path([Point(-10, 0), Point(10, 0)], wg_width).transformed(t1))
    self.cell.shapes(LayerPinRecN).insert(Text("opt1", t1, 0.3/dbu, -1))
    
    t = Trans(angle_vector(pts[-1]-pts[-2])/90, False, pts[-1])
    self.cell.shapes(LayerPinRecN).insert(Path([Point(-10, 0), Point(10, 0)], wg_width).transformed(t))
    self.cell.shapes(LayerPinRecN).insert(Text("opt2", t, 0.3/dbu, -1))
    ...
    shape = self.cell.shapes(LayerDevRecN).insert(text)
    t = Trans(0, False, pt6)
    text = Text( 'pinOrder:opt1 opt2',t, 0.1*wg_width, -1)
    text.halign=halign
    shape = self.cell.shapes(LayerDevRecN).insert(text)